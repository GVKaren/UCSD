...
import org.moeaframework.algorithm.pso.GMR;
import org.moeaframework.algorithm.pso.MOVPSO;
...
public class StandardAlgorithms extends AlgorithmProvider {
		...
		try {
			if (name.equalsIgnoreCase("MOEAD") ||
				...
				return newSMPSO(typedProperties, problem);
                        } else if (name.equalsIgnoreCase("MOVPSO")) {
				return newMOVPSO(typedProperties, problem);
                        } else if (name.equalsIgnoreCase("GMR")) {
				return newGMR(typedProperties, problem);
	...
	private Algorithm newSMPSO(TypedProperties properties, Problem problem) {
        ...
        /**
	 * Returns a new {@link MOVPSO} instance.
	 * 
	 * @param properties the properties for customizing the new {@code MOVPSO}
	 *        instance
	 * @param problem the problem
	 * @return a new {@code MOVPSO} instance
	 */
	private Algorithm newMOVPSO(TypedProperties properties, Problem problem) {
		if (!checkType(RealVariable.class, problem)) {
			throw new FrameworkException("unsupported decision variable type");
		}
		...
		return new MOVPSO(problem, populationSize, archiveSize,
				mutationProbability, distributionIndex);
	}
	/**
	 * Returns a new {@link GMR} instance.
	 * 
	 * @param properties the properties for customizing the new {@code GMR}
	 *        instance
	 * @param problem the problem
	 * @return a new {@code GMR} instance
	 */
	private Algorithm newGMR(TypedProperties properties, Problem problem) {
		if (!checkType(RealVariable.class, problem)) {
			throw new FrameworkException("unsupported decision variable type");
		}
		...
		return new GMR(problem, populationSize, archiveSize,
				mutationProbability, distributionIndex);
	}
